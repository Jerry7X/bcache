...
struct cache_sb {
	...
	struct {
		...
		uint16_t	nr_in_set;//总的cache设备数量
		uint16_t	nr_this_dev;//cache设备在cache_set中的编号
	};
	struct {
		/* Backing devices */
		uint64_t	data_offset;
        //扇区编号，从16开始，前面8k作为元数据
		/*
...
struct cached_dev {
	...
	struct bcache_device	disk;//bcache磁盘
	struct block_device	*bdev;//back磁盘
