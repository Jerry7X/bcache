...
int bch_open_buckets_alloc(struct cache_set *c)
{
	...
	for (i = 0; i < 6; i++) {
		...
		if (!b)
			return -ENOMEM;
        //ÕâÀïÎªÊ²Ã´Ã»ÓÐ¶Ôdata_buckets×ö³õÊ¼»¯
...
found:
	if (!ret->sectors_free && KEY_PTRS(alloc)) {
		//ÔÚÕâÀï²Å³õÊ¼»¯sectors_free£¬Æäµ¥Î»ÊÇÒÔsector¼ÆËãµÄ
			      ...
			      struct search *s)
{
    ...
    //ºÃ°É Õâ¶ÎÒâË¼ºÃÏñÊÇËµÈç¹ûalloc.key.ptr²»Îª¿Õ ÔòÖ¸ÏòÐÂ·ÖÅäµÄbucket
    ...
    //ÎªÉ¶²»ÊÇ¸ù¾ÝÐèÇóÀ´ÄØ Ö±½ÓÄÃ³öÀ´ÓÃ
	for (i = 0; i < KEY_PTRS(&b->key); i++)
		k->ptr[i] = b->key.ptr[i];
	...
	//keyµÄptrÊôÐÔÀ´×ÔÓëopen_bucketµÄkey(Ò²¾ÍÊÇbucket gen)
...
static void bch_insert_data_loop(struct closure *cl)
{
	...
	do {
		...
		k = op->keys.top;
		bkey_init(k);
		SET_KEY_INODE(k, op->inode);//high.virtual_device
		SET_KEY_OFFSET(k, bio->bi_sector);

        //bio_sectorsµÃµ½bioÇëÇóµÄÉÈÇøÊýÁ¿
        ...
        //Èç¹ûsplit£¬¾ÍÓÐ¶à¸ökey£¬·ñÔòÖ»ÓÐÒ»¸ö¡£
		n->bi_rw |= REQ_WRITE;
		bch_submit_bbio(n, op->c, k, 0);
	} while (n != bio);
    //µ½ÕâÀïÔÚcache deviceÖÐ²åÈëÊý¾Ý¾ÍÍê³ÉÁË
	op->insert_data_done = true;
	continue_at(cl, bch_journal, bcache_wq);
	//×¢ÒâÕâ¸öºêreturnÁË£¬²»»áÖ´ÐÐÏÂÃæµÄerr²¿·Ö
err:
	...
	if (s->write) {
		 ...
		 * rest of the write.
		 */
		 //write²Ù×÷Ö±½ÓÊ§°Üå ·µ»ØÊ²Ã´´íÎó¸øÉÏ²ãå
		op->skip = true;
		return bio_invalidate(cl);
	} else {
		/*
...
static void request_read_done_bh(struct closure *cl)
{
	...
	//ÓÐÎ´ÃüÖÐ£¬ÐèÒª½«Êý¾Ý¿½±´µ½cache dev
...
static void request_write(struct cached_dev *dc, struct search *s)
{
    ...
    //Óëgc keysÊÇ·ñoverlap
    //Èç¹ûÓÐ£¬Ôò½«Æä´Ógc keysÖÐÉ¾³ý
	bch_keybuf_check_overlapping(&s->op.c->moving_gc_keys, &start, &end);

	check_should_skip(dc, s);
	down_read_non_owner(&dc->writeback_lock);

    //Óëwb keysÊÇ·ñoverlap
	if (bch_keybuf_check_overlapping(&dc->writeback_keys, &start, &end)) {
		s->op.skip	= false;
		s->writeback	= true;
	}
    //×îÖÕÊÇ·ñoverlap»¹µÃ¿´btree
